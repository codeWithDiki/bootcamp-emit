@extends('layouts.app')
@section('content')
<div x-data="{showForm:false,modalUpdate:false, modalDetail:false, orderData:{id:false,customer_name:false,customer_phone:false,customer_address:false,qty:false,name:false,price:false,weight:false,total:false}}">
    <div class="max-w-5xl mx-auto mt-5">
        <h1 class="mb-5">Order List : </h1>
        <div class="my-10 p-5 border border-gray-500 rounded-md">
            <div x-show="!showForm">
              <button @click="showForm=!showForm" class="bg-green-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-3">Create Order</button>
              <livewire:orders-table />
            </div>
            <div x-show="showForm">
              <button @click="showForm=!showForm" class="bg-red-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-3 mb-3">Cancel</button>
              <livewire:edit-order />
            </div>
        </div>
    </div>
